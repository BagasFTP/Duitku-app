...
export default function BudgetAlertBell() {
    ...
    // seenKeys     : hanya menghilangkan badge (alert sudah dibaca)
    // dismissedKeys: menghilangkan alert dari panel ("Hapus Notifikasi")
    ...
    // Panel: semua alert kecuali yang sudah di-dismiss
    const panelAlerts    = alerts.filter(a => !dismissedKeys.includes(alertKey(a)));
    const panelOverCount = panelAlerts.filter(a => a.is_over).length;
    const panelNearCount = panelAlerts.filter(a => !a.is_over).length;

    // Badge: alert yang belum dibaca (belum di seenKeys) dan belum di-dismiss
    const unreadAlerts = panelAlerts.filter(a => !seenKeys.includes(alertKey(a)));
    const showBadge    = unreadAlerts.length > 0;
    ...
    const handleOpen = () => {
        setOpen(o => !o);
        // Tandai semua panel alert sebagai "sudah dibaca" → badge hilang
        const newSeenKeys = [...new Set([...seenKeys, ...panelAlerts.map(alertKey)])];
        setSeenKeys(newSeenKeys);
        localStorage.setItem(`${storageKey}-seenKeys`, JSON.stringify(newSeenKeys));
    };

    // Hapus Notifikasi: dismiss semua alert di panel
    const handleClear = () => {
        const newDismissed = [...new Set([...dismissedKeys, ...panelAlerts.map(alertKey)])];
                    ...
                    <div className="max-h-72 overflow-y-auto px-3 pb-3 space-y-2">
                        {panelAlerts.length === 0 ? (
                            <div className="py-6 flex flex-col items-center gap-2 text-slate-400">
                                ...
                                <p className="text-xs">Tidak ada notifikasi anggaran</p>
                            </div>
                        ) : panelAlerts.map((alert: BudgetAlert, i: number) => (
                            <div
                                key={i}
